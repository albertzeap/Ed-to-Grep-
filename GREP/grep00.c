...
void commands(void) {  unsigned int *a1;  int c, temp;  char lastsep;
  for (;;) {
    ...
    case 'e':  setnoaddr(); if (vflag && fchange) { fchange = 0;  error(Q); } filename(c);  init(); //Case e opens the file, allowing it to be searched
    ...
    caseread: //reads the contents of the file, can possibly be transformed into a function
